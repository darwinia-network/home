  ...
  return (
    <div>
      ...
      <div className={"flex mt-[1.5625rem] lg:mt-[2.8125rem]"}>
        ...
        <a className={"text-white hover:opacity-70"} target="_blank" href={link.url} rel="noreferrer">
          {link.title}
...
const getPaperHeaders = (paper: Paper, t: TFunction<"translation", undefined>) => {
  ...
  return (
    <>
      ...
      <div className={"title lg:title-lg lg:text-white text-white mt-[1.25rem]"}>
      ...
      <div className={"text-white mt-[1.25rem]"}>{authors}</div>
      <div className={"flex flex-wrap mt-[1.25rem]"}>{tags}</div>
    </>
  );
};
